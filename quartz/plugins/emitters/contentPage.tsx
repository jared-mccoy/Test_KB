...
import { FilePath, isRelativeURL, joinSegments, pathToRoot, simplifySlug, resolveRelative } from "../../util/path";
  ...
  return {
    ...
    async emit(ctx, content, resources): Promise<FilePath[]> {
      ...
      for (const [tree, file] of content) {
        ...
        const originalBacklinks = file.data.links || [];
        ...
        const backlinkFiles = allFiles.filter((file) => file.links?.includes(simplifySlug(slug)));
        const backlinks = backlinkFiles.map(f => ({
          title: f.frontmatter?.title || simplifySlug(f.slug!),
          slug: resolveRelative(slug, f.slug!),
        }));

        // Add backlinks to fileData
        file.data.backlinks = backlinks;
